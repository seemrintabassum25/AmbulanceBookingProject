...
def book_ambulance(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Get the selected ambulance type from the form
            selected_type = request.POST.get('ambulance_type', 'basic')

            # Find available ambulance based on user's selection
            available_ambulance = Ambulance.objects.filter(
                status='available',
                ambulance_type=selected_type
            ).first()

            # If no ambulance of selected type is available, try any available
            if not available_ambulance:
                available_ambulance = Ambulance.objects.filter(status='available').first()
                if available_ambulance:
                    messages.warning(request,
                                     f'No {selected_type} ambulance available. Assigned {available_ambulance.get_ambulance_type_display()} ambulance instead.')
